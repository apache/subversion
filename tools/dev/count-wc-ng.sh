...
WC_ACCESS=`ls *.[ch] | grep -Ev '(deprecated|entries)' | xargs grep svn_wc_adm_access_t | wc -l`
WC_ENTRY=`ls *.[ch] | grep -Ev '(deprecated|entries)' | xargs grep svn_wc_entry_t | wc -l`
