      ...
      if (prop_cl || old_v1_with_cl)
        {
          ...
          valstr_native = prop_cl ? prop_cl : content_length;
#if APR_CHARSET_EBCDIC
          SVN_ERR (svn_utf_cstring_from_utf8(&valstr_native,
                                             valstr_native,
                                             pool));
#endif
      ...
      if (text_cl)
        {
          ...
          svn_boolean_t is_delta = (delta && 
                                    strcmp (delta, TRUE_STR) == 0);
          valstr_native = text_cl;
#if APR_CHARSET_EBCDIC
          SVN_ERR (svn_utf_cstring_from_utf8(&valstr_native, valstr_native, pool));
#endif
...
#if !APR_CHARSET_EBCDIC
          valstr_native = content_length;
#else
          SVN_ERR (svn_utf_cstring_from_utf8(&valstr_native, content_length,
                                             pool));
#endif
          cl_value = svn__atoui64 (valstr_native) - actual_prop_length;

          if (cl_value || 
              ((node_kind = apr_hash_get (headers,
                                          SVN_REPOS_DUMPFILE_NODE_KIND,
                                          APR_HASH_KEY_STRING))
               && strcmp (node_kind, FILE_STR) == 0)
             )
            SVN_ERR (parse_text_block (stream,
                                       cl_value,
                                       FALSE,
      ...
      if (content_length && ! old_v1_with_cl)
        {
          ...
          valstr_native = content_length;
          valstr_native2 = prop_cl ? prop_cl : NULL;
          valstr_native3 = text_cl ? text_cl : NULL;
#if APR_CHARSET_EBCDIC
          SVN_ERR (svn_utf_cstring_from_utf8(&valstr_native, valstr_native,
                                             pool));
          if (valstr_native2)
            SVN_ERR (svn_utf_cstring_from_utf8(&valstr_native2,
                                               valstr_native2,
                                               pool));
          if (valstr_native3)
            SVN_ERR (svn_utf_cstring_from_utf8(&valstr_native3,
                                               valstr_native3,
                                               pool));
#endif
          remaining =
            svn__atoui64 (valstr_native) -
            (valstr_native2 ? svn__atoui64 (valstr_native2) : 0) -
            (valstr_native3 ? svn__atoui64 (valstr_native3) : 0);
...
#if APR_CHARSET_EBCDIC
  /* Feedback to stdout during a load on ebcdic
   * platforms is encoded in ebcdic. */
  if(nb->path)
    SVN_ERR (svn_utf_cstring_from_utf8(&nb_path_native, nb->path, pool));
#endif
