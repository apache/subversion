...
def create_python_hook_script (hook_path, hook_script_code):
  ...
  if sys.platform == 'win32':
    ...
    # This will run the python script, but since it uses 'system' the return value isn't captured
    # but instead is the return value of 'system' itself, likely always success.
    # file_append (hook_path,
    #              "cd %s\nsystem \"PYTHON233/PYTHON PROGRAM('%s.py')\"" % (hook_path.replace('\\', '/')[2:], os.path.basename(hook_path)))
    # So instead we just stick with using scripts.    
    file_append (hook_path, hook_script_code)
