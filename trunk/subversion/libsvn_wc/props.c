...
build_present_props (apr_hash_t *props, apr_pool_t *pool)
{
  ...
  cachable = svn_cstring_split (SVN_WC__CACHABLE_PROPS, SVN_UTF8_SPACE_STR,
                                TRUE, pool);
      ...
      if (apr_hash_get (props, proptolookfor, APR_HASH_KEY_STRING) != NULL)
        {
          ...
          svn_stringbuf_appendcstr (present_props, SVN_UTF8_SPACE_STR);
        }
    }
...
string_contains_prop (const char *string, const char *propname)
{
  ...
  while (place)
    {      
      if (place[proplen] == SVN_UTF8_SPACE || place[proplen] == 0)
        return TRUE;
