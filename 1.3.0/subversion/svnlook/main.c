...
#define MODIFIED_STR \
        "\x4d\x6f\x64\x69\x66\x69\x65\x64"
        /* "Modified" */
...
#pragma convert(37)
  ...
  if (name)
    ...
    *label = APR_PSPRINTF2 (pool, "%s\t%s (rev %ld)",
                            path, datestr, rev);
  return SVN_NO_ERROR;
}
  ...
  if (node->kind == svn_node_file)
    {
      ...
      else if (c->diff_copy_from && node->action == SVN_UTF8_A && is_copy)
        {
          if (node->text_mod)
            {
              do_diff = TRUE;
      ...
      else if (! c->no_diff_added && node->action == SVN_UTF8_A)
        {
          do_diff = TRUE;
          orig_empty = TRUE;
          SVN_ERR (prepare_tmpfiles (&orig_path, &new_path, &binary,
      ...
      else if (! c->no_diff_deleted && node->action == SVN_UTF8_D)
        {
          do_diff = TRUE;
          SVN_ERR (prepare_tmpfiles (&orig_path, &new_path, &binary,
            ...
            apr_pool_t *pool)
{
  ...
  SVN_ERR (SVN_CMDLINE_PRINTF (pool, "%s%s",
                               full_paths ? path : svn_path_basename (path,
                                                                      pool),
                               is_dir && strcmp (path, SVN_UTF8_FSLASH_STR)
                               ? SVN_UTF8_FSLASH_STR : ""));
...
#else
  /* On ebcdic platforms just output utf-8. */
  prop_value_native = apr_pstrdup(pool, prop_value->data);
#endif
