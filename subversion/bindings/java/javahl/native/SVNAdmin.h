...
{
public:
	void verify(const char *path, Outputer &messageOut, Revision &revisionStart, Revision &revisionEnd);
	void setLog(const char *path, Revision &revision, const char *message, bool bypassHooks);
	...
	void load(const char *path, Inputer &dataIn, Outputer &messageOut, bool ignoreUUID, bool forceUUID, const char *relativePath);
	...
	void dump(const char *path, Outputer &dataOut, Outputer &messageOut, Revision &revsionStart, Revision &RevisionEnd, bool incremental);
	void deltify(const char *path, Revision &start, Revision &end);
	void create(const char *path, bool ignoreUUID, bool forceUUID, const char *configPath);
	SVNAdmin();
	virtual ~SVNAdmin();
	jlong getCppAddr();
	void finalize();
...
#endif // !defined(AFX_SVNADMIN_H__9AD95B26_47BF_4430_8217_20B87ACCE87B__INCLUDED_)
