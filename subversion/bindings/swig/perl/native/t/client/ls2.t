...
diag 'Testing r0';
foreach my $args (([$repo_url, 0, 0, 1],
		   [ { path_or_url => $repo_url, peg_revision => 0,
		       revision => 0, recurse => 1 } ])) {

    $dirents = $ctx->ls2(@{ $args });
    isa_ok($dirents, 'HASH');
    is_deeply($dirents, { }, 'r0 is empty');
}

diag 'Testing r1';
foreach my $args (([$repo_url, 1, 1, 1],
		   [ { path_or_url => $repo_url, peg_revision => 1,
		       revision => 1, recurse => 1 } ])) {
...
foreach my $args ((["$repo_url/A", 1, 1, 0],
		   [ { path_or_url => "$repo_url/A", peg_revision => 1,
		       revision => 1, recurse => 0 } ],
	           [ { path_or_url => "$repo_url/A", peg_revision => 1,
		       revision => 1 } ], )) {
