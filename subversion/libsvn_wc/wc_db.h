...
/* Structures which mirror the tables in wc-metadata.sql.  For detailed
   descriptions of each field, see that file. */
typedef struct {
  apr_int64_t id;
  apr_int64_t wc_id;
...
typedef struct {
  ...
  apr_int64_t changelist_id;
} svn_wc__db_actual_node_t;

typedef struct {
  apr_int64_t id;
  apr_int64_t wc_id;
  const char *name;
} svn_wc__db_changelist_t;
