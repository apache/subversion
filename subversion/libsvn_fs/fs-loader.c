...
svn_fs_lock (svn_lock_t **lock, svn_fs_t *fs, const char *path,
             ...
             apr_pool_t *pool)
{
  return fs->vtable->lock (lock, fs, path, comment, force, timeout, pool);  
}
...
svn_error_t *
svn_fs_get_lock_from_token (svn_lock_t **lock, svn_fs_t *fs, const char *token,
                           apr_pool_t *pool)
{
  return fs->vtable->get_lock_from_token (lock, fs, token, pool);
}
