...
/* Delete node revision ID (with predecessor id PRED_ID, if any) from
   ...
   If PRED_ID is not provided, also delete the record for this ID's
   node ID from the `node-origins' index table (which is typically
   only done if the caller things that ID points to the only node
   revision ID in its line of history).
