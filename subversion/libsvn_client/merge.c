                                  ...
                                  apr_pool_t *pool)
{
  ...
  SVN_ERR(svn_wc__get_entry_versioned(&target_entry, merge_b->ctx->wc_ctx,
                                      local_abspath, svn_node_unknown,
                                      FALSE, FALSE, pool, pool));
...
/* Helper for do_directory_merge().
   ...
   URL1, REVISION1, URL2, REVISION2, NOTIFY_B, MERGE_B, and POOL are
  ...
  for (i = 1; i < notify_b->children_with_mergeinfo->nelts; i++)
    {
      ...
      if (deleted_rangelist->nelts || added_rangelist->nelts)
        {
          size_t merge_target_len = strlen(merge_b->target_abspath);
...
/* Helper for record_mergeinfo_for_dir_merge().
   ...
   true if the caller is recording a reverse merge and false otherwise.
   RANGELIST is the set of revisions being merged from MERGEINFO_PATH to
  ...
  if (nbr_skips > 0)
    {
      ...
      SVN_ERR(update_wc_mergeinfo(merge_b->target_abspath, target_entry,
                      ...
                      apr_pool_t *scratch_pool)
{
  ...
  const char *abs_parent_path = svn_dirent_dirname(local_abspath, scratch_pool);
  svn_node_kind_t kind;
  svn_depth_t depth;
  ...
  if (path_is_merge_target
          ...
          )
    {
      ...
      child->path = apr_pstrdup(wb->pool, local_abspath);
                                   ...
                                   apr_pool_t *pool)
{
  ...
  if (!(child->absent
          || (child->switched
              && strcmp(merge_cmd_baton->target_abspath, child->path) != 0)))
    return SVN_NO_ERROR;
...
/* Helper for do_directory_merge()
   ...
   copy tree rooted at MERGE_CMD_BATON->TARGET.
                  ...
                  if (!child_of_noninheritable)
                    {
                      ...
                      SVN_ERR(svn_dirent_get_absolute(&con_abspath,
                                              child_of_noninheritable->path,
                                              iterpool));
                      ...
                      if (!merge_cmd_baton->dry_run
                          && merge_cmd_baton->same_repos)
                        {
                          ...
                          SVN_ERR(svn_wc__get_entry_versioned(
                            ...
                            con_abspath, svn_node_unknown, FALSE, FALSE,
                            iterpool, iterpool));
              ...
              apr_pool_t *scratch_pool)
{
  ...
  SVN_ERR(svn_wc__get_entry_versioned(&entry, merge_b->ctx->wc_ctx,
  ...
  if (honor_mergeinfo)
    {
      ...
      SVN_ERR(svn_ra_reparent(merge_b->ra_session1, entry->url,
                              scratch_pool));
      ...
      if (filtered_rangelist->nelts
              ...
              || (apr_hash_count(notify_b->skipped_paths) == 0)))
        {
          ...
          apr_hash_set(merges, target_abspath, APR_HASH_KEY_STRING,
                       filtered_rangelist);
          SVN_ERR(update_wc_mergeinfo(target_abspath, entry, mergeinfo_path,
                                      merges, is_rollback, ctx, subpool));
        }
    }
  ...
  if (merge_b->paths_with_new_mergeinfo && !merge_b->dry_run)
    {
      ...
      for (hi = apr_hash_first(NULL, merge_b->paths_with_new_mergeinfo);
           ...
           hi = apr_hash_next(hi))
        {
          const char *path_with_new_mergeinfo = svn_apr_hash_index_key(hi);
          const char *pwnm_abspath;
          ...
          SVN_ERR(svn_dirent_get_absolute(&pwnm_abspath,
                                          path_with_new_mergeinfo, iterpool));
          SVN_ERR(svn_wc__get_entry_versioned(&path_entry,
                                              merge_b->ctx->wc_ctx,
                                              pwnm_abspath, svn_node_unknown,
                                              FALSE, FALSE,
                                              iterpool, iterpool));
...
/* Helper for do_directory_merge().
   ...
   TARGET_ENTRY, DEPTH, NOTIFY_B, and MERGE_B are all cascaded
          ...
          if (added_path_parent_propval
                        ...
                        SVN_MERGEINFO_NONINHERITABLE_STR))
            {
              ...
              SVN_ERR(svn_client__get_wc_mergeinfo(
                ...
                NULL, NULL, merge_b->ctx, iterpool));
              ...
              apr_pool_t *pool)
{
  ...
  SVN_ERR(svn_rangelist_merge(&(log_gap_baton->operative_ranges),
                              init_rangelist(revision - 1 , revision,
                                             TRUE, log_gap_baton->pool),
                              log_gap_baton->pool));
                           ...
                           apr_pool_t *scratch_pool)
{
  ...
  requested_ranges = init_rangelist(MIN(revision1, revision2),
                                    MAX(revision1, revision2),
                                    TRUE, scratch_pool);
          ...
          if (subtree_gap_ranges->nelts)
            {
              ...
              SVN_ERR(svn_client__path_relative_to_root(
                ...
                target_abspath, target_entry->repos, TRUE, NULL,
...
/* Perform a two-URL merge between URLs which are related, but neither
   ...
   TARGET_ABSPATH.  Other arguments are as in all of the public merge
   APIs.
