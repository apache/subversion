...
typedef struct txdelta_stream_baton_t
{
...
txdelta_md5_digest(void *baton)
{
  ...
  return svn_txdelta_md5_digest(b->txstream);
}
...
typedef struct open_txdelta_stream_baton_t
{
  ...
  unsigned char checksum_digest[APR_MD5_DIGESTSIZE];
                    ...
                    apr_pool_t *scratch_pool)
{
  ...
  svn_txdelta2(&txdelta_stream, svn_stream_empty(result_pool),
               contents, TRUE, result_pool);

  /* Retarget the checksum digest to our buffer, so that it would
   * have appropriate lifetime. */
  tsb = apr_pcalloc(result_pool, sizeof(*tsb));
  tsb->digest_buffer = b->checksum_digest;
  tsb->txstream = txdelta_stream;

  *txdelta_stream_p = svn_txdelta_stream_create(tsb, txdelta_next_window,
                                                txdelta_md5_digest,
                                                result_pool);
  return SVN_NO_ERROR;
}
            ...
            apr_pool_t *pool)
{
  ...
  apr_hash_t* properties;
  apr_hash_index_t *hi;
  open_txdelta_stream_baton_t otsb = {0};
  ...
  otsb.local_abspath = local_abspath;
  otsb.properties = properties;
  SVN_ERR(editor->apply_textdelta_stream(editor, file_baton, NULL,
                                         open_txdelta_stream, &otsb, pool));
  text_checksum = svn_checksum_to_cstring(
                    svn_checksum__from_digest_md5(otsb.checksum_digest, pool),
                    pool);
