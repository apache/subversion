                 ...
                 svn_boolean_t prop_mods)
{
  ...
  new_item->text_mods = text_mods;
  new_item->prop_mods = prop_mods;
                      ...
                      apr_pool_t *pool)
{
  ...
  svn_boolean_t do_add = FALSE, do_delete = FALSE, is_copy = FALSE;
  ...
  /* If a deletion is scheduled to occur, register this path as a
     committable thing. */
  if (do_delete)
    add_committable (committables, path, entry, FALSE, FALSE);

  /* If an add is scheduled to occur, register this path as a
     committable thing. */
  if (do_add)
    {
      ...
      add_committable (committables, path, entry, text_mod, prop_mod);
    }
      ...
      for (hi = apr_hash_first (subpool, entries); hi; hi = apr_hash_next (hi))
        {
          ...
          SVN_ERR (harvest_committables 
                   (committables, full_path,
                    ...
                    (svn_wc_entry_t *)val, 
                    is_copy ? TRUE : adds_only,
                    subpool));

          /* Truncate paths back to their pre-loop state. */
          svn_stringbuf_chop (full_path, full_path->len - path->len);
          if (! this_entry->url)
            svn_stringbuf_chop (this_url, this_url->len - url->len);
        }
    }
...
int svn_client__sort_commit_item_urls (const void *a, const void *b)
{
  ...
  return svn_path_compare_paths (item1->entry->url, item2->entry->url);
}
                                   ...
                                   apr_pool_t *pool)
{
  ...
  url = (((svn_client_commit_item_t **) ci->elts)[0])->entry->url;
  ...
  if (ci->nelts == 1)
  ...
  else
    {
      ...
      for (i = 1; i < ci->nelts; i++)
        {
          url = (((svn_client_commit_item_t **) ci->elts)[i])->entry->url;
