...
static svn_error_t *
out_of_date(const char *path, svn_node_kind_t kind, const char *txn_name)
{
             ...
             apr_pool_t *pool)
{
  ...
  if (SVN_IS_VALID_REVNUM(revision) && (revision < cr_rev))
    return out_of_date(full_path, kind, eb->txn_name);
          ...
          void **file_baton)
{
  ...
  if (SVN_IS_VALID_REVNUM(base_revision) && (base_revision < cr_rev))
    return out_of_date(full_path, svn_node_file, eb->txn_name);
  ...
  if (SVN_IS_VALID_REVNUM(db->base_rev))
    {
      ...
      if (db->base_rev < created_rev)
        return out_of_date(db->path, svn_node_dir, eb->txn_name);
    }
