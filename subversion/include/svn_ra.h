...
/**
 * Fetch the mergeinfo for @a paths (relative to the session URL) at
 * @a rev, and save it to @a mergeoutput.  @a mergeoutput is a mapping
 * of <tt>const char *</tt> target paths (from @a paths) to hashes
 * mapping merged-from paths (<tt>const char *</tt>) to revision range
 * lists (<tt>apr_array_header_t *</tt> with <tt>svn_merge_range_t
 * *</tt> elements), or @c NULL if there is no merge info available.
