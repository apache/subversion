 ...
 *
 * If the DISCOVER_CHANGED_PATHS flag was passed to get_log, then
 * CHANGED_PATHS contains as keys every path committed in REVISION;
 * else, CHANGED_PATHS is null.
 *
 * On the final call to this function in a given invocation of
 * get_log, LAST_CALL is set.
 */
typedef svn_error_t *(*svn_ra_log_message_receiver_t)
     (void *baton,
