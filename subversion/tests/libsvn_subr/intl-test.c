...
#define assert_l10n_okay(l10n, intl_val) \
  if ((l10n->value == NULL) != (intl_val == NULL) \
      || (l10n->value != NULL && intl_val != NULL \
          && apr_strnatcmp (l10n->value, intl_val) != 0)) \
    return fail(pool, "Expected value '%s' not equal to '%s' for " \
                "text '%s'", l10n->value, intl_val, l10n->key); \
  ...
  for (l10n = l10n_list; l10n->key != NULL; l10n++)
    {
      ...
      const char *intl_value /*= svn_intl_dgettext (PACKAGE_NAME, l10n->key);
                               assert_l10n_okay (l10n, intl_value)*/;

      intl_value = svn_intl_dlgettext (PACKAGE_NAME, l10n->locale, l10n->key);
      assert_l10n_okay (l10n, intl_value);
    }
