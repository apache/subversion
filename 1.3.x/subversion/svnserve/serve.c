                                       ...
                                       apr_pool_t *pool)
{
  ...
  /* If the authz request is for the empty path (ie. ""), replace it
     ...
     ACLs. */
  if (path && *path != SVN_UTF8_FSLASH)
    path = svn_path_join(SVN_UTF8_FSLASH_STR, path, pool);
  ...
  for (i = 0; i < path_revs->nelts; ++i)
    {
      ...
      if (err)
        ...
        {
          SVN_ERR(svn_ra_svn_write_tuple(conn, subpool, "w!", SUCCESS_STR));
  ...
  for (i = 0; i < unlock_tokens->nelts; i++)
    {
      ...
      if (err)
        ...
        SVN_ERR(svn_ra_svn_write_tuple(conn, subpool, "w(c)", SUCCESS_STR,
                                       path));
    }
