...
parser.add_option("-g", "--gccxml", dest="gccxml", action="store_true",
                  default=False,
                  help="Use the gccxml parser instead of the wraptypes parser")
...
if options.gccxml:
    os.system("cd %s && h2xml.py -c %s %s -o svn_all.xml" %
              (tempdir, f.name, flags))
    os.system("cd %s && xml2py.py %s -r '^svn.*|^apr.*|^SVN_.*|^APR_.*' "
              "svn_all.xml -o svn_all.py" % (tempdir, ldflags))
else:
    os.system("cd %s && %s -U __GNUC__ %s svn_all.h | grep -v '^#' > svn_all2.h" %
              (tempdir, cpp, flags))
    os.system("cd %s && python %s/wraptypes/wrap.py %s %s "
              "--include-symbols 'svn|apr|APR|SVN' "
              "--exclude-symbols '.*va_list.*' "
              "svn_all2.h -o svn_all.py" % (tempdir, os.getcwd(),
                                            flags, ldflags))
...
f = file("%s/svn_constants.h" % tempdir, "w")
...
cmd = "cd %s && %s %s svn_constants.h" % (tempdir, cpp, flags)
print cmd
